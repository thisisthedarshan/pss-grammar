...
activity_join_branch 
    : TOKEN_JOINBRANCH TOKEN_FLBRACE 
        label_identifier 
      (TOKEN_COMMA 
        label_identifier
      )* TOKEN_FRBRACE
    ;
activity_join_select 
    : TOKEN_JOINSELECT TOKEN_FLBRACE 
        expression 
      TOKEN_FRBRACE
    ;
activity_join_none 
    ...
    ;
activity_join_first 
    : TOKEN_JOINFIRST TOKEN_FLBRACE 
        expression 
      TOKEN_FRBRACE
    ;

activity_repeat_stmt 
    : (TOKEN_REPEAT TOKEN_FLBRACE (
        index_identifier 
      TOKEN_COLON)? 
        expression 
      TOKEN_FRBRACE 
    ...
    | (TOKEN_REPEAT 
        activity_stmt 
      TOKEN_WHILE TOKEN_FLBRACE 
        expression 
      TOKEN_FRBRACE TOKEN_SEMICOLON)
    ;

activity_foreach_stmt 
    : TOKEN_FOREACH TOKEN_FLBRACE (
        iterator_identifier 
      TOKEN_COLON )? 
        expression
      (TOKEN_SLBRACE 
        index_identifier 
      TOKEN_SRBRACE )? TOKEN_FRBRACE 
        activity_stmt
    ;

activity_select_stmt 
    : TOKEN_SELECT TOKEN_CLBRACE 
        select_branch 
        select_branch 
        select_branch* 
      TOKEN_CRBRACE
...
select_branch 
    : ((TOKEN_FLBRACE 
        expression 
      TOKEN_FRBRACE (TOKEN_FLBRACE 
        expression 
      TOKEN_FRBRACE)? TOKEN_COLON) 
    | (TOKEN_SLBRACE 
        expression 
      TOKEN_SRBRACE TOKEN_COLON))? 
        activity_stmt
    ;

activity_if_else_stmt 
    : TOKEN_IF TOKEN_FLBRACE 
        expression 
      TOKEN_FRBRACE 
        activity_stmt 
      (TOKEN_ELSE 
...
activity_match_stmt 
    : TOKEN_MATCH TOKEN_FLBRACE 
        match_expression 
      TOKEN_FRBRACE TOKEN_CLBRACE 
...
match_choice 
    ...
    | (TOKEN_DEFAULT TOKEN_COLON 
        activity_stmt
      ))
    ;

activity_replicate_stmt 
    : TOKEN_REPLICATE TOKEN_FLBRACE (
        index_identifier 
      TOKEN_COLON)? 
        expression 
      TOKEN_FRBRACE 
      (
        label_identifier 
      TOKEN_SLBRACE TOKEN_SRBRACE TOKEN_COLON)? 
        labeled_activity_stmt
    ;

activity_super_stmt 
    : TOKEN_SUPER TOKEN_SEMICOLON 
    ;

activity_atomic_block_stmt 
    : TOKEN_ATOMIC TOKEN_CLBRACE 
        activity_stmt* 
      TOKEN_CRBRACE
    ;
